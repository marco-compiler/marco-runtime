...
unsigned int ThreadPool::getNumOfThreads() const { return numOfThreads; }
  ...
  while (true) {
...

    {
      ...
      queueCondition.wait(lock,
                          [this] { return !jobs.empty() || shouldTerminate; });
...
namespace marco::runtime {
unsigned int ThreadPool::getNumOfThreads() const { return 1; }

void ThreadPool::async(const std::function<void()> &job) { job(); }
