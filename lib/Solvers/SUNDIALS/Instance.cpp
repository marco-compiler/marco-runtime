...
size_t VariableDimensions::rank() const { return dimensions.size(); }
...
bool VariableDimensions::isValid() const {
  return std::none_of(dimensions.begin(), dimensions.end(),
                      [](const auto &dimension) { return dimension == 0; });
}

VariableIndicesIterator::~VariableIndicesIterator() { delete[] indices; }

VariableIndicesIterator
VariableIndicesIterator::begin(const VariableDimensions &dimensions) {
...
VariableIndicesIterator
VariableIndicesIterator::end(const VariableDimensions &dimensions) {
  VariableIndicesIterator result(dimensions);
...
const uint64_t *VariableIndicesIterator::operator*() const { return indices; }
...
void VariableIndicesIterator::fetchNext() {
  ...
  assert(std::none_of(dimensions->begin(), dimensions->end(),
                      [](const auto &dimension) { return dimension == 0; }));
...
bool advanceVariableIndices(uint64_t *indices,
                            const VariableDimensions &dimensions) {
...
bool advanceVariableIndices(std::vector<uint64_t> &indices,
                            const VariableDimensions &dimensions) {
  return advanceVariableIndices(indices.data(), dimensions);
}

bool advanceVariableIndicesUntil(std::vector<uint64_t> &indices,
                                 const VariableDimensions &dimensions,
...
uint64_t getVariableFlatIndex(const VariableDimensions &dimensions,
...
uint64_t getEquationFlatIndex(const std::vector<int64_t> &indices,
                              const MultidimensionalRange &ranges) {
  return getFlatIndex(indices, ranges);
}

void getEquationIndicesFromFlatIndex(uint64_t flatIndex,
                                     std::vector<int64_t> &result,
                                     const MultidimensionalRange &ranges) {
  return getIndicesFromFlatIndex(flatIndex, result, ranges);
}

double getCellFromSparseMatrix(SUNMatrix matrix, uint64_t rowIndex,
