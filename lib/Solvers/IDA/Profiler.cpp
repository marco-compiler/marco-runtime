...
IDAProfiler::IDAProfiler() : Profiler("IDA") { registerProfiler(*this); }
...
void IDAProfiler::print() const {
  ...
  std::cerr << "Time spent on IDA steps: "
            << stepsTimer.totalElapsedTime<std::milli>() << " ms" << std::endl;

  std::cerr << "Time spent on computing the algebraic variables: "
            << algebraicVariablesTimer.totalElapsedTime<std::milli>() << " ms"
            << std::endl;
  ...
  std::cerr << "Time spent on computing the partial derivatives: "
            << partialDerivativesTimer.totalElapsedTime<std::milli>() << " ms"
            << std::endl;

  std::cerr << "Time spent on copying the variables from MARCO: "
            << copyVarsFromMARCOTimer.totalElapsedTime<std::milli>() << " ms"
            << std::endl;

  std::cerr << "Time spent on copying the variables into MARCO: "
            << copyVarsIntoMARCOTimer.totalElapsedTime<std::milli>() << " ms"
            << std::endl;
}
